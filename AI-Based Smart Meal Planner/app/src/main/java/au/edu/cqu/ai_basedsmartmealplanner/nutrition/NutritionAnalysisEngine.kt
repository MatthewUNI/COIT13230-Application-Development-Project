...
object NutritionAnalysisEngine {
    ...
    // Baseline nutritional reference estimates per standard serving based on AFCD categories[cite: 1]
    ...
    /**
     * Calculates the nutritional breakdown for an individual recipe[cite: 1].
     */
    fun analyzeRecipe(recipe: Recipe): NutritionInfo {
        ...
        for (ingredient in recipe.ingredients) {
            ...
            val matchedNutrients = ingredientNutritionTable.entries.firstOrNull { lower.contains(it.key) }?.value
    ...
    /**
     * Aggregates total nutritional intake across all meals in a meal plan[cite: 1].
     */
    fun analyzeDailyPlan(mealPlan: MealPlan): NutritionInfo {
        ...
        for (meal in mealPlan.dailyMeals) {
            val analysis = analyzeRecipe(meal)
